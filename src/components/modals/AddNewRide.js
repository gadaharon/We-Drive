...
import { addNewRidePost, getPosts } from '../../actions/postsAction';
...
export class AddNewRide extends Component {

  state = {
     desttinationTime: '', // <- Arival time of drive post
    selectedTime: '', // <- Departue time of the ride post
    selectedDate: '', // <- Departue date of the ride post (for single ride post)
    ...
    availableSeats: "", // <Seats available for ride post
    switchBtn: false,// Flag to declare single or constant ride post
    zIndexState: { from: 1, to: 0 }, // <- zIndex flag
    originLocation: "", // <- Departure location for ride post
    destinationLocation: "",// <- Destination location for ride post
    originCoords:'',// <- Departure coords for ride post
    destCoords:'', // <- Destination coords for ride post
  ...
  // Create New Ride Post Handler
  addNewRide = async () => {
    const user = JSON.parse(await AsyncStorage.getItem("user")); 
    const userId = user.id; // <- Get user id from AsyncStorage
    ...
    const objectToSend = {
      ...
      dateOrDays: switchBtn ? days.join(',') : selectedDate,
      time: selectedTime,
      availableSeats,
    ...
    this.props.addNewRidePost(objectToSend);
    this.props.getPosts();
    setTimeout(() => {
    ...
    <Modal visible={visable} onRequestClose={visabilityHandler}>
       ...
       >
         ...
         <Picker.Item label="Four Seats" value="4" />
              ...
              onPress={this.addNewRide}
              >
                <Text style={{textAlign:'center', fontSize: 19, color: '#3498db'}}>Create New Ride</Text>
              </TouchableOpacity>
...
export default connect(mapStateToProps, { addNewRidePost, getPosts })(AddNewRide);
